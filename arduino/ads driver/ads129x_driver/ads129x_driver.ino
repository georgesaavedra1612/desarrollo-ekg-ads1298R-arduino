...
void setup() {  
  ...
  pinMode(22,OUTPUT);   //En este pin habrá una señal cuadrada sincronizada con la onda R, esta irá al pin del interruptor 4.
  ...
  WiredSerial.println("Declaración de comandos OK"); 
  detectActiveChannels();
  ...
  */                                             
  ...
  for (int prueba = 1; prueba <=5000; prueba++){ //prueba <=5000
...
inline void sendSample(void) { 
  ...
  //WiredSerial.print(",");
  //WiredSerial.print(convert_two_complement(serialBytes[9], serialBytes[10], serialBytes[11]));  //ch3 OK 
  //WiredSerial.print(",");
  ...
  //PSe enviará la señal correspondiente al canal 2:
  int standard = map((convert_two_complement(serialBytes[6], serialBytes[7], serialBytes[8])), -3000, 3000, 0, 1023);
...
//LO SIGUIENTE FUNCIONA BIEN PARA TRANSMITIR A BLUETOOTH:
  /*
  byte Data[4];
  ...
  Data[2]=bateria;
  Data[3]=frecu;
  WiredSerial.write(Data[0]);
  WiredSerial.write(Data[1]);
  WiredSerial.write(Data[2]);
  WiredSerial.write(Data[3]);
  */
}
